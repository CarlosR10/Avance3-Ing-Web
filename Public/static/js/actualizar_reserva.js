const registroFormulario = document.getElementById('actForm');
registroFormulario.addEventListener('submit', (event) => {
    event.preventDefault(); // Prevent default form submission
    actualizarReservaSubmit();
});

async function actualizarReservaSubmit() {
    let registro = {
    ...
    let urlLocal = 'http://localhost/preorder/update';
    let url = 'https://addled-radiuses.000webhostapp.com/preorder/update'
    let options = {
        method: 'POST',
        headers: {
    ...
    if (responseData.success){
        ...
        responseData.message = "<p><strong> Ups! </strong> No ha sido posible actualizar su reserva.</p>";
